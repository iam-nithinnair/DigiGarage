...
--    to avoid infinite recursion when policies query the same table)
...
-- Admins can view all profiles (uses safe function)
...
-- Admins can update any profile (uses safe function)
...
-- 6. Admin RLS policies for models table (allow admins to read/delete any model)
